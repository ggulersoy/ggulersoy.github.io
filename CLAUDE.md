...
**⚠️ The CV PDF needs a `/Title` in its metadata.** The LaTeX build emitted `/Title ()` (empty), so Google indexed the PDF under the **anchor text of the link pointing at it** ("Download PDF") and showed that as the search-result headline. The committed PDF was patched in place to `/Title (Curriculum Vitae, Gürcan Zeren Gülersoy)` (+ `/Author`, `/Subject`), keeping the info dictionary the same byte length so the XRef **stream** stays valid (the file has no plain xref table to renumber; the `/PTEX.Fullbanner` entry was dropped to free the bytes). **This regresses on every recompile** — fix it at the source with `\hypersetup{pdftitle={Curriculum Vitae, Gürcan Zeren Gülersoy}, pdfauthor={Gürcan Zeren Gülersoy}, pdfsubject={Curriculum Vitae}}` in the LaTeX repo. No local PDF tooling (`qpdf`, `exiftool`, `gs`, `pikepdf`) is installed on this machine; verify a patched PDF with `mdls -name kMDItemTitle -name kMDItemNumberOfPages` plus a Read-tool page render.
